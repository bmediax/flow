...
import type { AIConfiguration, AIProvider } from "./state";
...
/**
 * Fetches the list of models available for the given OpenAI API key.
 * Only returns models the account has access to (avoids 403 on restricted models).
 */
...
async function translateDocument(
	...
	for (const batch of batches) {
		...
		const combinedText = batch.map((node) => node.textContent).join(DELIMITER);
...
interface SpineItem {
	href?: string;
	url?: string;
	load: (loader: (path: string) => Promise<ArrayBuffer>) => Promise<void>;
	document?: Document;
}
...
export async function translateEpub(
	...
	try {
		...
		if (!provider || !model) {
			throw new TranslationError(
				"AI provider and model are required. Please check Settings.",
				"MISSING_PROVIDER",
			);
		}
		...
		onProgress?.({
			total: 1,
			current: 0,
			currentSection: "Loading ePub file...",
		});

		let epub: Awaited<ReturnType<typeof fileToEpub>>;
		try {
		...
		const sections: SpineItem[] =
			(spine as { spineItems?: SpineItem[] }).spineItems ?? [];
