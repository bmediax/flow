...
import type { Rendition, Book } from '@flow/epubjs'
import type { Location } from '@flow/epubjs/types/rendition'
...
export class BookTab extends BaseTab {
  ...
  async render(el: HTMLDivElement) {
    ...
    ;(this.rendition as any).on('relocated', (loc: Location) => {
    ...
    ;(this.rendition as any).on('attached', (...args: any[]) => {
      console.log('attached', args)
    })
    ;(this.rendition as any).on('started', (...args: any[]) => {
      console.log('started', args)
    })
    ;(this.rendition as any).on('displayed', (...args: any[]) => {
      console.log('displayed', args)
    })
    ;(this.rendition as any).on('rendered', (section: ISection, view: any) => {
      console.log('rendered', [section, view])
      this.section = ref(section)
      this.iframe = ref(view.window) as any
    })
    ;(this.rendition as any).on('selected', (...args: any[]) => {
      console.log('selected', args)
    })
    ;(this.rendition as any).on('removed', (...args: any[]) => {
