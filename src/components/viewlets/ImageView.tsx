...
export const ImageView: React.FC<PaneViewProps> = (props) => {
  ...
  const sections = (focusedBookTab?.sections as any as ISection[] | undefined)?.filter(
    (s: ISection) => s.images.length,
  )
...
const Block: React.FC<BlockProps> = ({ section }) => {
  ...
  const resources = (focusedBookTab?.epub as any)?.resources
