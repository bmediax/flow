...
import { TranslationError, translateEpub } from "../translate";
...
export function HomePage() {
	...
	// Handle PWA file launch (File System Access / Launch Queue API)
	useEffect(() => {
		if ("launchQueue" in window && "LaunchParams" in window) {
			const launchQueue = (window as Window & { launchQueue?: { setConsumer(fn: (params: { files: { getFile(): Promise<File> }[] }) => void): void } }).launchQueue;
...
const Library: React.FC = () => {
		...
		db?.books.toArray().then(async (books) => {
			for (const remoteFile of remoteFiles) {
				...
				await dbx
					...
					.then((d) => {
						const blob = (d.result as unknown as { fileBlob: Blob }).fileBlob;
	...
	const selectedBooks = [...selectedBookIds]
		...
		.filter((b): b is BookRecord => b != null);
									...
									onClick={async () => {
										...
										try {
											if (db) {
												...
												await db.covers.bulkDelete(bookIds);
												await db.files.bulkDelete(bookIds);
											}

											// Remove from Dropbox if synced; don't fail the whole delete if this throws
											try {
												await mutateRemoteFiles(
													async (data) => {
														...
														return data?.filter(
															(f) => !selectedBooks.some((b) => b.name === f.name),
														);
													},
													{ revalidate: false },
												);
												...
												const msg =
													remoteErr instanceof Error
														? remoteErr.message
														: String(remoteErr ?? "Unknown error");
												console.warn("Could not delete books from cloud:", msg);
											}
										} catch (err) {
											const message =
												err instanceof Error && err.message
													...
													: typeof err === "string"
														? err
														: "Failed to delete books. Try again.";
											console.error("Failed to delete books:", err ?? message);
											alert(message);
										}
									}}
								>
...
const Book: React.FC<BookProps> = ({
	...
	const [, setTranslationError] = useTranslationError();
	...
	const handleTranslate = async (e: React.MouseEvent) => {
		...
		if (activeTranslation) {
			alert(
				"A translation is already in progress. Please wait for it to complete.",
			);
			return;
		}
		...
		const providerName =
			settings.ai.provider === "anthropic" ? "Anthropic" : "OpenAI";
		...
		// Clear any previous translation error when starting a new one
		...
		(async () => {
			try {
				...
				if (!fileRecord) {
					setTranslationError(
						"Book file not found. Please re-import the book.",
					);
	...
	return (
		<div className="relative flex flex-col">
			{/* biome-ignore lint/a11y/useSemanticElements: card contains a nested Translate <button>; HTML forbids button inside button */}
			<div
				role="button"
				tabIndex={0}
				aria-label={select ? "Toggle selection" : `Open ${book.name}`}
				className="border-inverse-on-surface relative w-full border bg-transparent p-0 text-left focus:outline-none"
				onClick={async () => {
					if (select) {
						toggle(book.id);
					} else {
			...
			>
				...
				{/* biome-ignore lint: cover is data URL or blob, next/image not suitable */}
				<img
			...
			{isEditingName ? (
				...
				<button
					type="button"
					className="line-clamp-2 text-on-surface-variant typescale-body-small lg:typescale-body-medium mt-2 w-full cursor-pointer text-left hover:text-on-surface focus:outline-none"
					title={book.name}
